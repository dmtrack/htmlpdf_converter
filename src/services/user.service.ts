...
class UserService {
    async signUp(user: IUserSignUp) {
        ...
        const newUser: IUserDto = await User.create({name: name, email: email,
            password: hashpass, activationLink: activationLink, blocked: false, isActivated: false, avatarUrl: 'www.image.com/1' 
        });
        await mailService.sendActivationMail({email, activationLink})
      
        const userDto = new UserDto(newUser)
        const tokens = tokenService.generateTokens({...userDto})
        const accessRight = await Access.create({access: 'user',  userId: userDto.id})
        const token: IToken = await tokenService.saveToken(userDto.id, tokens.refreshToken)
        await User.update({ refreshToken: token.id, accessId: accessRight.id }, { where: { id: userDto.id } });
